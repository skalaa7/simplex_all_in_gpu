...
void makeMatrix(float wv[ROWSIZE*COLSIZE])
{
	...
	fstream myFile;
    myFile.open("../../simplex_input_generator/baza5000.txt",ios::in); //open in read mode
	if(myFile.is_open())
    {
        for(int j = 0; j < ROWSIZE; j++)
        {
...
int main(int argc, char *argv[])
{
	...
	cl::Buffer b_newRow, b_pivotColVal, b_wv,b_opti_unb,b_rat,b_pivotRow;//,b_unbounded,b_pivotRow;
	...
	b_opti_unb = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(int)*2);
	...
	b_pivotRow = cl::Buffer(context,CL_MEM_READ_WRITE,sizeof(int)*1);
	  ...
	  cl::make_kernel< int, int, cl::Buffer, cl::Buffer, cl::Buffer,cl::Buffer,cl::Buffer> simplex_calc(prog_simplex_calc, "simplex_calc");
    ...
    #endif
    {
    	//start = omp_get_wtime();
    	count[1]++;
    	simplex_calc(cl::EnqueueArgs(queue, cl::NDRange(1)), ROWSIZE, COLSIZE, b_newRow, b_pivotColVal, b_wv,b_opti_unb,b_pivotRow);
    	queue.finish();
    	cl::copy(queue, b_opti_unb, opti_unb, opti_unb+2);
    	if(opti_unb[0] || opti_unb[1])
    		break;
    	pivoting(cl::EnqueueArgs(queue, cl::NDRange(ROWSIZE)),
	   	    ROWSIZE, COLSIZE, b_newRow, b_pivotColVal, b_wv,b_pivotRow);
	queue.finish();
	//elapsed2  += omp_get_wtime() - start;
    }
    //float solVar[NUMOFVAR];
    cl::copy(queue, b_wv, wv, wv+(ROWSIZE-1)*COLSIZE+(COLSIZE));
    int unbounded=opti_unb[1];
    //
    //Write results
    if(unbounded)
    {
